...
class AtonQodefImport {
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'qode_options_aton', $options);
    ...
    function qode_admin_import() {
        if (qode_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($aton_qodef_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'aton_qodef_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'aton_qodef_enqueue_admin_styles');
...
function qode_init_import_object(){
    global $aton_qodef_import_object;
    $aton_qodef_import_object = new AtonQodefImport();
}

add_action('init', 'qode_init_import_object');

if(!function_exists('aton_qodef_dataImport')){
    function aton_qodef_dataImport(){
        global $aton_qodef_import_object;

        if ($_POST['import_attachments'] == 1)
            $aton_qodef_import_object->attachments = true;
        else
            $aton_qodef_import_object->attachments = false;

        $folder = "aton/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $aton_qodef_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_qode_dataImport', 'aton_qodef_dataImport');
}

if(!function_exists('aton_qodef_widgetsImport')){
    function aton_qodef_widgetsImport(){
        global $aton_qodef_import_object;
        ...
        $aton_qodef_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_qode_widgetsImport', 'aton_qodef_widgetsImport');
}

if(!function_exists('aton_qodef_optionsImport')){
    function aton_qodef_optionsImport(){
        global $aton_qodef_import_object;
        ...
        $aton_qodef_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_qode_optionsImport', 'aton_qodef_optionsImport');
}

if(!function_exists('aton_qodef_otherImport')){
    function aton_qodef_otherImport(){
        global $aton_qodef_import_object;
        ...
        $aton_qodef_import_object->import_options($folder.'options.txt');
        $aton_qodef_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $aton_qodef_import_object->import_menus($folder.'menus.txt');
        $aton_qodef_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_qode_otherImport', 'aton_qodef_otherImport');
}