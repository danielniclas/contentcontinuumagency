...
if( ! class_exists('AtonQodefSidebar') ){
	
	class AtonQodefSidebar{
		...
		function __construct(){
			...
			$this->title = esc_html__('Custom Widget Area','aton');
		...
		function load_assets(){
			...
			wp_enqueue_script('qode_sidebar' , QODE_ROOT . '/framework/admin/assets/js/qodef-sidebar.js');
			wp_enqueue_style( 'qode_sidebar' , QODE_ROOT . '/framework/admin/assets/css/qodef-sidebar.css');
		}
		...
		function template_add_widget_field(){
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '".esc_html__('Enter Name of the new Widget Area', 'aton')."' name='qode-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='".esc_html__('Add Widget Area', 'aton')."' />";
			echo "\n    ".$nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function add_sidebar_area(){
			if(!empty($_POST['qode-add-widget'])){
				...
				$name = rtrim($this->get_name($_POST['qode-add-widget']));
		...
		function register_custom_sidebars(){
			...
			$args = apply_filters('aton_qodef_custom_widget_args', $args);
