<?php
namespace AtonQodef\Modules\Shortcodes\PieCharts\PieChartDoughnut;
...
class PieChartDoughnut implements ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ($i = 1; $i <= $this->chartFields; $i++) {
			...
			$chart_fields[] = array(
				'type' => 'textfield',
                'heading' => sprintf( esc_html__( 'Chart Value %s', 'aton' ), $i),
				'param_name' => "chart_value_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'colorpicker',
                'heading' => sprintf( esc_html__( 'Chart Color %s', 'aton' ), $i),
				'param_name' => "chart_color_" . $i,
			);
			$chart_fields[] = array(
				'type' => 'textfield',
                'heading' => sprintf( esc_html__( 'Chart Legend %s', 'aton' ), $i),
		...
		vc_map( array(
			'name' => esc_html__('Select Pie Chart 3 (Doughnut)', 'aton'),
			'base' => $this->getBase(),
			'category' => 'by SELECT',
			...
			'params' => array_merge(
				array(
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__('Height', 'aton'),
						'param_name' => 'height',
					),
				),
				$chart_fields
	...
	public function render($atts, $content = null) {
		...
		$html = aton_qodef_get_shortcode_module_template_part('templates/pie-chart-doughnut', 'piecharts/piechartdoughnut', '', $params);
