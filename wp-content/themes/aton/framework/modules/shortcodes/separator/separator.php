<?php
namespace AtonQodef\Modules\Shortcodes\Separator;
...
class Separator implements ShortcodeInterface{
	...
	public function vcMap() {
		...
		vc_map(
			array(
				'name' => esc_html__('Select Separator', 'aton'),
				'base' => $this->base,
				'category' => 'by SELECT',
				...
				'params' => array(
					...
					array(
						'type' => 'dropdown',
						'heading' => esc_html__('Border Style', 'aton'),
						'param_name' => 'border_style',
						'value' => array(
							esc_html__('Default', 'aton') => '',
							esc_html__('Dashed', 'aton') => 'dashed',
							esc_html__('Solid', 'aton') => 'solid',
							esc_html__('Dotted', 'aton') => 'dotted'
						)
					),
					array(
						'type' => 'textfield',
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__('Thickness (px)', 'aton'),
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__('Top Margin', 'aton'),
					...
					array(
						'type' => 'textfield',
						'heading' => esc_html__('Bottom Margin', 'aton'),
	...
	public function render($atts, $content = null) {
		...
		$html = aton_qodef_get_shortcode_module_template_part('templates/separator-template', 'separator', '', $params);
	...
	private function getSeparatorStyle($params) {
		...
		if ($params['width'] !== '') {
			if(aton_qodef_string_ends_with($params['width'], '%') || aton_qodef_string_ends_with($params['width'], 'px')) {
		...
		if ($params['top_margin'] !== '') {
			if(aton_qodef_string_ends_with($params['top_margin'], '%') || aton_qodef_string_ends_with($params['top_margin'], 'px')) {
		...
		if ($params['bottom_margin'] !== '') {
			if(aton_qodef_string_ends_with($params['bottom_margin'], '%') || aton_qodef_string_ends_with($params['bottom_margin'], 'px')) {
