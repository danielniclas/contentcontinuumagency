...
class AtonQodefInstagramWidget extends AtonQodefWidget {
    protected $params;
    public function __construct() {
        parent::__construct(
            'qodef_instagram_widget',
            esc_html__( 'Select Instagram Widget', 'aton'),
            array( 'description' => esc_html__( 'Display instagram images', 'aton' ) )
        );
    ...
    protected function setParams() {
        $this->params = array(
            ...
            array(
                'name' => 'tag',
                'type' => 'textfield',
                'title' => esc_html__('Tag', 'aton')
            ),
            array(
                'name' => 'number_of_photos',
                'type' => 'textfield',
                'title' => esc_html__('Number of photos', 'aton')
            ),
            array(
                'name' => 'number_of_cols',
                'type' => 'dropdown',
                'title' => esc_html__('Number of columns', 'aton'),
                'options' => array(
                    '2' => esc_html__('Two', 'aton'),
                    '3' => esc_html__('Three', 'aton'),
                    '4' => esc_html__('Four', 'aton'),
                    '6' => esc_html__('Six', 'aton'),
                    '9' => esc_html__('Nine', 'aton'),
                )
            ),
            array(
                'name' => 'image_size',
                'type' => 'dropdown',
                'title' => esc_html__('Image Size', 'aton'),
                'options' => array(
                    'thumbnail' => esc_html__('Small', 'aton'),
                    'low_resolution' => esc_html__('Medium', 'aton'),
                    'standard_resolution' => esc_html__('Large', 'aton')
                )
            ),
            array(
                'name' => 'transient_time',
                'type' => 'textfield',
                'title' => esc_html__('Images Cache Time', 'aton')
            ),
        );
    }
            ...
            foreach ($images_array as $image) { ?>
                <li>
                    <a itemprop="url" href="<?php echo esc_url($instagram_api->getHelper()->getImageLink($image)); ?>" target="_blank">
                        <?php echo aton_qodef_kses_img($instagram_api->getHelper()->getImageHTML($image, $image_size)); ?>
                    </a>
                </li>
            <?php } ?>
            </ul>
