...
if(!function_exists('aton_qodef_get_title')) {
    /**
     * Loads title area HTML
     */
    function aton_qodef_get_title() {
        $id = aton_qodef_get_page_id();

        extract(aton_qodef_title_area_height());
        extract(aton_qodef_title_area_background());
        ...
        if(($meta_temp = get_post_meta($id, "qodef_show_title_area_meta", true)) !== ""){
            ...
            $show_title_area = aton_qodef_options()->getOptionValue('show_title_area') == 'yes' ? true : false;
        }
        ...
        if(($meta_temp = get_post_meta($id, "qodef_title_area_type_meta", true)) !== ""){
            ...
            $type = aton_qodef_options()->getOptionValue('title_area_type');
        }
        ...
        if(($meta_temp = get_post_meta($id, "qodef_title_area_enable_breadcrumbs_meta", true)) !== ""){
            ...
            $enable_breadcrumbs = aton_qodef_options()->getOptionValue('title_area_enable_breadcrumbs') == 'yes' ? true : false;
        }

        //check if breadcrumbs are enabled on page first, then in options
        if(($meta_temp = get_post_meta($id, "qodef_title_area_enable_separator_meta", true)) !== ""){
            $enable_separator = $meta_temp == 'yes' ? true : false;
        }else {
            $enable_separator = aton_qodef_options()->getOptionValue('title_area_enable_separator') == 'yes' ? true : false;
        }
        ...
        $parameters = apply_filters('aton_qodef_title_area_height_params', $parameters);

        aton_qodef_get_module_template_part('templates/title', 'title', '', $parameters);
    }
}

if(!function_exists('aton_qodef_get_title_text')) {
    /**
     * Function that returns current page title text. Defines aton_qodef_title_text filter
     ...
     */
    function aton_qodef_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'aton');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'aton') . " " . get_the_author();
        }
        ...
        elseif (is_singular('post')) {
            if (aton_qodef_options()->getOptionValue('blog_single_title_in_title_area') == "yes") {
        ...
        elseif (is_404()) {
            ...
            if(aton_qodef_options()->getOptionValue('404_title') != "") {
                //get it from options
                $title = aton_qodef_options()->getOptionValue('404_title');
            } else {
                //get default 404 page title
                $title = esc_html__('404 - Page not found', 'aton');
            }
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(aton_qodef_is_woocommerce_installed() && (is_shop() || is_singular('product'))) {
        ...
        elseif(aton_qodef_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        $title = apply_filters('aton_qodef_title_text', $title);

        return $title;
    }
}

if(!function_exists('aton_qodef_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see aton_qodef_get_title_text()
     */
    function aton_qodef_title_text() {
        echo aton_qodef_get_title_text();
    }
}

if(!function_exists('aton_qodef_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     *
     */
    function aton_qodef_subtitle_text() {
        $id = aton_qodef_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('aton_qodef_custom_breadcrumbs')) {
    /**
     ...
     * @see is_front_page()
     * @see is_category()
     * @see aton_qodef_is_product_category()
     * @see get_search_query()
     */
    function aton_qodef_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = aton_qodef_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="qodef-delimiter" '.aton_qodef_get_inline_style($bread_style).'>&nbsp;/&nbsp;</span>'; // delimiter between crumbs
        $home = esc_html__('Home', 'aton'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="qodef-current" '.aton_qodef_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a itemprop="url" '.aton_qodef_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a itemprop="url" '.aton_qodef_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a itemprop="url" '.aton_qodef_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="qodef-breadcrumbs"><div class="qodef-breadcrumbs-inner"><a itemprop="url" '.aton_qodef_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || aton_qodef_is_product_category()) {
                ...
                $output .= $before . esc_html__('Search results for ', 'aton') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a itemprop="url" '.aton_qodef_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a itemprop="url" '.aton_qodef_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
                ...
                $output .= '<a itemprop="url" '.aton_qodef_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<a itemprop="url" '.aton_qodef_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a>';
                ...
                $output .= $before . esc_html__('Posts tagged ','aton') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                global $authordata;
                $output .= $before . esc_html__('Articles posted by ','aton') . $authordata->display_name . $after;
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'aton') . ' ' . get_query_var('paged') . ")" . $after;
...
if(!function_exists('aton_qodef_get_title_area_height')) {

    /**
     * Function that returns title height
     **/
    function aton_qodef_get_title_area_height() {
        ...
        if(($meta_temp = get_post_meta($id, 'qodef_title_area_type_meta', true)) != '') {
            ...
            $title_type = aton_qodef_options()->getOptionValue('title_area_type');
        }

        $title_height = 490; // default title height without header height
        ...
        } elseif(aton_qodef_options()->getOptionValue('title_area_height') !== '') {
            $title_height = aton_qodef_options()->getOptionValue('title_area_height');
        }

        return apply_filters('aton_qodef_title_area_height', $title_height);
    }
}

if(!function_exists('aton_qodef_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/

    function aton_qodef_get_title_content_padding() {
        return apply_filters('aton_qodef_title_content_padding', 0);
    }
}


if(!function_exists('aton_qodef_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function aton_qodef_title_area_height() {
        $id = aton_qodef_get_page_id();
        $title_height_and_padding = array();
        $title_height          = aton_qodef_get_title_area_height();
        $header_height_padding = aton_qodef_get_title_content_padding();
        ...
        if(($meta_temp = get_post_meta($id, "qodef_title_area_background_image_responsive_meta", true)) != "") {
            ...
            $is_img_responsive = aton_qodef_options()->getOptionValue('title_area_background_image_responsive');
        }

        if(($meta_temp = get_post_meta($id, "qodef_title_area_vertial_alignment_meta", true)) !== '') {
            $title_vertical_alignment = $meta_temp;
        }else{
            $title_vertical_alignment = aton_qodef_options()->getOptionValue('title_area_vertial_alignment');;
        }
...
if(!function_exists('aton_qodef_title_area_background')) {
    /**
     * Function that returns title background style be applied in template
     **/

    function aton_qodef_title_area_background() {
        $id = aton_qodef_get_page_id();
        $show_title_img = true;
        ...
        if(($meta_temp = get_post_meta($id, "qodef_title_area_background_color_meta", true)) != ""){
            ...
            $background_color = aton_qodef_options()->getOptionValue('title_area_background_color');
        }
        ...
        if(($meta_temp = get_post_meta($id, "qodef_title_area_background_image_meta", true)) != ""){
            ...
            $background_image = aton_qodef_options()->getOptionValue('title_area_background_image');
        }

        //check for background image width
        $background_image_width = "";
        if($background_image !== ''){
            $background_image_width_dimensions_array = aton_qodef_get_image_dimensions($background_image);
