...
class AtonQodefLike {
	...
	private function __construct() {
		...
		add_action('wp_ajax_aton_qodef_like', array( $this, 'ajax'));
		add_action('wp_ajax_nopriv_aton_qodef_like', array( $this, 'ajax'));
	}
		...
		switch($action) {
			...
			case 'get':
				...
				if(isset($_COOKIE['qodef-like_'. $post_id])){
					$icon = '<i class="ion-heart" aria-hidden="true"></i>';
				}else{
					$icon = '<i class="ion-heart" aria-hidden="true"></i>';
				}
				if( !$like_count ){
			...
			case 'update':
				...
				//if($type != 'portfolio_list') {
					$return_value = "<i class='ion-heart' aria-hidden='true'></i><span>" . $like_count . "</span>";
	...
	public function add_like() {
		...
		$title = esc_html__('Like this', 'aton');
		if( isset($_COOKIE['qodef-like_'. $post->ID]) ){
			$class = 'qodef-like liked';
			$title = esc_html__('You already like this!', 'aton');
		}
	...
	public function add_like_portfolio_list($portfolio_project_id){
		...
		return '<a class="'. $class .'" data-type="portfolio_list" id="qodef-like-'. $portfolio_project_id .'-'. $rand_number.'" title="'. $title .'">'. $output .'</a>';
	}
...
if ( !function_exists( 'aton_qodef_create_like' ) ) {

    function aton_qodef_create_like() {
        AtonQodefLike::get_instance();
    }

    add_action('after_setup_theme', 'aton_qodef_create_like');
}