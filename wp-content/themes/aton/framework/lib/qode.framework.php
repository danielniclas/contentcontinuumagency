...
class AtonQodefFramework {
    ...
    private function __construct() {
        $this->qodeOptions = AtonQodefOptions::get_instance();
        $this->qodeMetaBoxes = AtonQodefMetaBoxes::get_instance();
    }
	...
	public function setSkin(AtonQodefSkinAbstract $skinObject) {
		$this->skin = $skinObject;
	}
}

/**
 * Class AtonQodefSkinManager
 *
...
/**
 * Class AtonQodefSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AtonQodefSkinAbstract {
    /**
     * @var string
     */
    protected $skinName;
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if($template !== '') {
            include(aton_qodef_find_template_path('framework/admin/skins/'.$this->skinName.'/templates/'.$template.'.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
		<script>
			window.qodeSCIcon = '<?php echo aton_qodef_get_skin_uri().'/assets/img/admin-logo-icon.png'; ?>';
			window.qodeSCLabel = '<?php echo esc_html(ucfirst($this->skinName)); ?> <?php esc_html_e('Shortcodes', 'aton'); ?>';
		</script>
	<?php }
...
class AtonQodefOptions {
    ...
    public function getOptionValue($key) {
        ...
        if(array_key_exists($key, $aton_qodef_options)) {
            return $aton_qodef_options[$key];
...
/*
   Class: AtonQodefAdminPage
   A class that initializes Qode Admin Page
*/
class AtonQodefAdminPage implements iAtonQodefLayoutNode {
    ...
    function __construct($slug="", $title="", $icon = "") {
        $this->layout = array();
        $this->factory = new AtonQodefFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iAtonQodefRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: AtonQodefMetaBoxes
...
/*
   Class: AtonQodefMetaBox
   A class that initializes Qode Meta Box
*/
class AtonQodefMetaBox implements iAtonQodefLayoutNode {
	...
	public function setTitle($label) {
		...
		return $aton_qodef_Framework->getSkin()->getSkinLabel().' '.$label;
 	}
}

if ( ! function_exists( 'aton_qodef_init_framework_variable' ) ) {

    function aton_qodef_init_framework_variable() {

        global $aton_qodef_Framework;

        $aton_qodef_Framework = AtonQodefFramework::get_instance();
        $qodeSkinManager = new AtonQodefSkinManager();
        $aton_qodef_Framework->setSkin($qodeSkinManager->getSkin());
    }

    add_action('aton_qodef_before_options_map', 'aton_qodef_init_framework_variable' );
}
?>