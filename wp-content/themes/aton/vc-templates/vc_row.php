...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(aton_qodef_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.aton_qodef_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($scrolling_text != '') {
    ...
    $scrolling_text_html = '';
    $scrolling_text_html .= '<div class="qodef-marquee-holder" '. aton_qodef_get_inline_style(implode(';', $scrolling_text_style)) .' ><ul>';
    $scrolling_text_html .= '<li class="qodef-marquee">';
    ...
    $after_wrapper_open .= $scrolling_text_html;

    $css_classes[] = 'qodef-scrolling-text-enabled';
}
...
if($full_screen_section_height == 'yes'){
	$css_classes[] =  'qodef-full-screen-height-section';
	$after_wrapper_open .= '<div class="qodef-full-screen-height-outer">';
...
if($arrow_down == 'yes') {
    ...
    $arrow_icon_style = array();

    if($arrow_down_color !== ''){
        $arrow_icon_style[] = 'color:'.  $arrow_down_color;
    }

    $arrow_icon .= '<div class="qodef-section-arrow-icon">';
    $arrow_icon .= '<a class="qodef-anchor" itemprop="url" href="#' . $arrow_down_anchor .'" '. aton_qodef_get_inline_style(implode(';', $arrow_icon_style)) .'>';
    $arrow_icon .= '<span class="arrow_down"></span>';
    $arrow_icon .= '</a>';
    $arrow_icon .= '</div>';

    $css_classes[] = 'qodef-with-anchor-icon';

    $before_wrapper_close .= $arrow_icon;
}
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="qodef-mobile-video-image" ' . aton_qodef_get_inline_attr($video_mobile_style, 'style') . '></div>';
	}
	$video_output .= '<div ' . aton_qodef_get_class_attribute($video_overlay_class) . aton_qodef_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="qodef-video-wrap">';
	$video_output .= '<video class="qodef-video" width="1920" height="800" '. aton_qodef_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . aton_qodef_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="'.esc_html__('video thumb','aton').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
